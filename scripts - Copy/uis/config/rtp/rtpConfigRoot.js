...
uiManager.addUI(config.uiNames.Config.RTP, "RTP configuration", (player)=>{
    ...
    modalForm.textField("RTP Cost - Amount", "Enter a price...", (configAPI.getProperty("RTPCost") ? configAPI.getProperty("RTPCost") : 0).toString())
    ...
    }), internalOptions.findIndex(_=>_ == prismarineDb.economy.getCurrency(configAPI.getProperty("RTPCurrency")).scoreboard));
    modalForm.textField("RTP Radius", "Enter a radius...", (configAPI.getProperty("RTPRadius") ? configAPI.getProperty("RTPRadius") : 1000).toString())
    modalForm.show(player, false, (player, response)=>{
        configAPI.setProperty("RTPEnabled", response.formValues[0]);
        if(/^\d+$/.test(response.formValues[1])) configAPI.setProperty("RTPCost", parseInt(response.formValues[1]));
        configAPI.setProperty("RTPCurrency", internalOptions[response.formValues[2]]);
        if(/^\d+$/.test(response.formValues[3])) configAPI.setProperty("RTPRadius", parseInt(response.formValues[3]));

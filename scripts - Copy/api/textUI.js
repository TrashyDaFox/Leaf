import { ButtonState, Player, system, world, InputButton } from "@minecraft/server";
        ...
        let interval = system.runInterval(()=>{
            ...
            for(let i = 0; i < opts.length; i++) {
                ...
                if(opt.type == "goto") {
                    ...
                    if(!otherUI) continue;
                    textForm.push(`${currSelectedOpt == i ? "§b" : "§7"}> ${opt.back ? "Back" : otherUI.data.name} §r${currSelectedOpt == i ? "§b" : "§7"}<`);
                } else if(opt.type == "action") {
                    textForm.push(`${currSelectedOpt == i ? "§b" : "§7"}> ${opt.name} §r${currSelectedOpt == i ? "§b" : "§7"}<`);
                }
            }

            if(player.inputInfo.getButtonState(InputButton.Sneak) == ButtonState.Released) {

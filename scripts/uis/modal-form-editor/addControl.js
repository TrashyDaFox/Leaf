...
uiManager.addUI(versionData.uiNames.Modal.AddControl, "ADD CONTROL", (player, id, controlIndex = -1, data = {}, tempData = {})=>{
    ...
    if(!data.type) {
        ...
        form.title("Select control type")
        form.button(`Divider\n(Separator line)`, `textures/items/add_divider`, (player)=>{
            data.type = `divider`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Header\n(Display text)`, `textures/azalea_icons/add_header`, (player)=>{
            data.type = `header`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Label\n(Display text)`, `textures/azalea_icons/add_label`, (player)=>{
            data.type = `label`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Text Field\n(Using command)`, `textures/azalea_icons/GUIMaker/ModalsV2/add text box`, (player)=>{
            data.type = `text-field-using-command`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Toggle\n(Using tag)`, `textures/azalea_icons/GUIMaker/ModalsV2/Toggle tag`, (player)=>{
            data.type = `toggle-using-tag`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Toggle\n(Using command)`, `textures/azalea_icons/GUIMaker/ModalsV2/Toggle command`, (player)=>{
            data.type = `toggle-using-command`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Dropdown\n(Using command)`, `textures/azalea_icons/GUIMaker/ModalsV2/dropdown`, (player)=>{
            data.type = `dropdown-using-command`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Dropdown\n(Using tags)`, `textures/azalea_icons/GUIMaker/ModalsV2/Dropdown tags`, (player)=>{
            data.type = `dropdown-using-tags`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Dropdown\n(Using score)`, `textures/azalea_icons/GUIMaker/ModalsV2/Dropdown score`, (player)=>{
            data.type = `dropdown-using-score`
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Slider\n(Using score)`, `textures/azalea_icons/GUIMaker/ModalsV2/Slider score`, (player)=>{
            data.type = `slider-using-score`
            data.step = 1;
            uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        })
        form.button(`Slider\n(Using command)`, `textures/azalea_icons/GUIMaker/ModalsV2/Slider command`, (player)=>{
    ...
    if(data.type == "text-field-using-command") {
        ...
        */
        ...
        form.button(`Set Display`, null, (player)=>{
            ...
            modal.textField("Label", "Type a label here", data.label ? data.label : undefined);
            modal.textField("Placeholder", "Type a placeholder here", data.placeholder ? data.placeholder : undefined);
            modal.toggle("Preserve Value", data.preserve ?? false);
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        ...
        form.button(`Set Action`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Action", "Type an action here", data.action ? data.action : undefined)
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        ...
        if(data.label && data.placeholder && data.action) {
            form.button(`${controlIndex == -1 ? "Create" : "Edit"}`, null, (player)=>{
                createOrEdit();
            })
        }
        form.show(player, false, (player, response)=>{})
    ...
    if(data.type == "toggle-using-tag") {
        ...
        form.button(`Set Tag`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Tag", "Type a tag here", data.tag ? data.tag : undefined)
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
    ...
    if(data.type == "toggle-using-command") {
        ...
        form.button(`Set Action`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Action", "Type an action here", data.action ? data.action : undefined)
            modal.dropdown("Response Format", ["true/false", "1/0"].map(_=>{
                return {
                    option: _,
                    callback() {}
                }
            ...
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
                data.action = response.formValues[0]
                data.responseFormat = response.formValues[1] == 1 ? "1/0" : "true/false"
                uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
            })
        })
    ...
    if(data.type == "dropdown-using-command") {
        ...
        form.button(`Add Option`, null, (player)=>{
            ...
            modal.textField("Command Override (optional)", "Custom command for this option", undefined)
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
                data.options.push({
        ...
        if(data.options.length > 0) {
            ...
            form.button(`Rearrange Options`, null, (player)=>{
                let modal = new ActionForm()
                modal.body("Select an option to move, then choose its new position")
                for(let option of data.options) {
                    modal.button(option.label, null, ()=>{
                        let moveModal = new ActionForm()
                        moveModal.body(`Where should "${option.label}" be moved to?`)
                        
                        for(let i = 0; i < data.options.length; i++) {
                            if(data.options[i] !== option) {
                                moveModal.button(`Move before "${data.options[i].label}"`, null, ()=>{
                                    data.options = data.options.filter(o => o !== option)
                                    data.options.splice(i > data.options.indexOf(option) ? i - 1 : i, 0, option)
                                    uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
                                })
                            }
                        }
        ...
        form.button(`Set Base Command`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Base Command", "Type base command here", data.action ? data.action : undefined)
            modal.dropdown("Response Format", ["Dropdown Label", "Index (0)", "Index (1)"].map(_=>{
                return {
                    option: _,
                    callback() {}
                }
    ...
    if(data.type == "dropdown-using-score") {
        ...
        form.button(`Set Options (${data.options.length})`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Options (separate with comma)", "Option 1, Option 2, Option 3", data.options.map(o => o.label).join(", "))
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
                data.options = response.formValues[0].split(",").map(label => ({label: label.trim()}))
                uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
            })
        })
    ...
    if(data.type == "slider-using-score") {
        ...
        form.button(`Set Range`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Minimum", "Minimum value", data.min?.toString() ?? "0")
            modal.textField("Maximum", "Maximum value", data.max?.toString() ?? "10")
            modal.textField("Step", "Step value", data.step?.toString() ?? "1")
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)
        ...
        if(data.label && data.objective && data.min !== undefined && data.max !== undefined) {
            form.button(`${controlIndex == -1 ? "Create" : "Edit"}`, null, (player)=>{
                createOrEdit();
            })
        }
    ...
    if(data.type == "slider-using-command") {
        ...
        if(data.label && data.action && data.min !== undefined && data.max !== undefined) {
            form.button(`${controlIndex == -1 ? "Create" : "Edit"}`, null, (player)=>{
                createOrEdit();
            })
        }
    ...
    if(data.type == "header" || data.type == "label") {
        ...
        form.button(`Set Text`, null, (player)=>{
            let modal = new ModalForm();
            modal.textField("Text", "Type text here", data.text ? data.text : undefined);
            modal.show(player, false, (player, response)=>{
                if(response.canceled) return uiManager.open(player, versionData.uiNames.Modal.AddControl, id, controlIndex, data, tempData)

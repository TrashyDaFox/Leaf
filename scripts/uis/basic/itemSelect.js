...
uiManager.addUI(config.uiNames.Basic.ItemSelect, "Item selector", (player, callback)=>{
    ...
    function getItemLore(item) {
        ...
        return {
            name: `§e${parseItemID(item.typeId)}${item.nameTag ? ` §6(${item.nameTag}§r§6)` : ``}`,
            lore,
            enchanted
        }
    }
    for(let i = 0;i < 9 * 6;i++) {
        chest.button(i, `§cX`, [], `textures/blocks/glass_gray`, 1, false, ()=>{
            uiManager.open(player, config.uiNames.Basic.ItemSelect, callback)
        })
    }
    for(let i = 0;i < 9;i++) {
    ...
    for(let i = 0;i < hotbar.length;i++) {
        ...
        chest.button((4 * 9) + i, name, lore, typeIdToID.has(item.typeId) ? item.typeId : `textures/azalea_icons/NoTexture`, item.amount, enchanted, ()=>{
            let confText = [`§l§eIS THIS INFO CORRECT?`,` `]
            confText.push(name)
            confText.push(` `)
            confText.push(...lore)
            uiManager.open(player, config.uiNames.Basic.Confirmation, confText.join('\n§r'), ()=>{
    ...
    for(let i = 0;i < items.length;i++) {
        ...
        let {name, lore, enchanted} = getItemLore(item)
        chest.button(9 + i, name, lore, typeIdToID.has(item.typeId) ? item.typeId : `textures/azalea_icons/NoTexture`, item.amount, enchanted, ()=>{
            let confText = [`§l§eIS THIS INFO CORRECT?`,` `]
            confText.push(name)
            confText.push(` `)
            confText.push(...lore)
    ...
    chest.button(0, `§cBack`, [`§7Click to go back`], `textures/azalea_icons/2`, 1, false, ()=>{
        callback(player, null)
    })

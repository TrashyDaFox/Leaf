...
uiManager.addUI(versionData.uiNames.ChatRanks.Config, "a", (player)=>{
    ...
    modalForm.title("Chat ranks config")
    modalForm.textField("Default Rank", "Member", configAPI.getProperty("DefaultRank"))
    // modalForm.textField("Default Name Color", "Member", configAPI.getProperty("DefaultRank"))
    modalForm.dropdown("Default Name Color", colors.getColorNamesColored().map(_=>{
        return {option: _}
    }), Math.max(0, colors.getColorCodes().indexOf(configAPI.getProperty("DefaultNameColor"))))
    modalForm.dropdown("Default Bracket Color", colors.getColorNamesColored().map(_=>{
        return {option: _}
    }), Math.max(0, colors.getColorCodes().indexOf(configAPI.getProperty("DefaultBracketColor"))))
    modalForm.dropdown("Default Message Color", colors.getColorNamesColored().map(_=>{
        return {option: _}

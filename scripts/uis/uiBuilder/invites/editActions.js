...
uiManager.addUI(versionData.uiNames.InviteManager.EditActions, "", (player, id, key)=>{
    let doc = uiBuilder.db.getByID(id)
    if(!doc.data[key]) doc.data[key] = [];
    let actionForm = new ActionForm();
    actionForm.title(key)
    actionForm.button(`§aAdd Action\n§7Add a new action`, `textures/azalea_icons/1`, (player)=>{
        let modalForm = new ModalForm();
        modalForm.textField("Action", "Example: /say hi", "")
        modalForm.show(player, false, (player, response)=>{
            if(response.canceled) return uiManager.open(player, versionData.uiNames.InviteManager.EditActions, id, key)
            doc.data[key].push(response.formValues[0])
            uiBuilder.db.overwriteDataByID(doc.id, doc.data)
            uiManager.open(player, versionData.uiNames.InviteManager.EditActions, id, key)
        })
    })
    for(let i = 0;i < doc.data[key].length;i++) {
        ...
        actionForm.button(`${action}`, null, (player)=>{
            ...
            testForm.button(`Edit Action`, null, (player)=>{
                let modalForm = new ModalForm();
                modalForm.textField("Action", "Example: /say hi", doc.data[key][i])
                modalForm.show(player, false, (player, response)=>{
                    if(response.canceled) return uiManager.open(player, versionData.uiNames.InviteManager.EditActions, id, key)
            ...
            testForm.button(`Move Up`, `textures/azalea_icons/Up`, (player)=>{
                array_move(doc.data[key], i, i - 1 < 0 ? 0 : i - 1)
                uiBuilder.db.overwriteDataByID(doc.id, doc.data)
                uiManager.open(player, versionData.uiNames.InviteManager.EditActions, id, key)
            })
            testForm.button(`Move Down`, `textures/azalea_icons/Down`, (player)=>{
                array_move(doc.data[key], i, i + 1 >= doc.data[key].length ? doc.data[key].length - 1 : i + 1)
                uiBuilder.db.overwriteDataByID(doc.id, doc.data)
                uiManager.open(player, versionData.uiNames.InviteManager.EditActions, id, key)
            })
            testForm.button(`Delete`, `textures/azalea_icons/Delete`, (player)=>{

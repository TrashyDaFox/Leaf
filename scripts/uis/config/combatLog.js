...
uiManager.addUI(versionData.uiNames.CombatLogConfig, "A", player=>{
    ...
    modalForm.textField("Exit combat cooldown (seconds)", "10", configAPI.getProperty("CLogSecCooldown").toString())
    modalForm.toggle("Enable enter message", configAPI.getProperty("CLogEnterMessageEnabled"))
    modalForm.textField("Enter combat message", "You have entered combat!", configAPI.getProperty("CLogEnterMessage"))
    modalForm.toggle("Enable exit message", configAPI.getProperty("CLogExitMessageEnabled"))
    modalForm.textField("Exit combat message", "You have left combat!", configAPI.getProperty("CLogExitMessage"))
    modalForm.toggle("Disable UIs", configAPI.getProperty("CLogDisableUIs"))
    modalForm.toggle("Disable commands", configAPI.getProperty("CLogDisableCommands"))
    modalForm.show(player, false, (player, response)=>{
        ...
        configAPI.setProperty("CLogSecCooldown", getNum(response.formValues[1]))
        configAPI.setProperty("CLogEnterMessageEnabled", getNum(response.formValues[2]))
        configAPI.setProperty("CLogEnterMessage", getNum(response.formValues[3]))
        configAPI.setProperty("CLogExitMessageEnabled", getNum(response.formValues[4]))
        configAPI.setProperty("CLogDisableUIs", getNum(response.formValues[5]))
        configAPI.setProperty("CLogDisableCommands", getNum(response.formValues[6]))

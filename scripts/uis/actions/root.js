...
uiManager.addUI("add_instruction", "a", (player, group)=>{
    ...
    actionForm.button(`§dNew Comment\n§7Label ur stuff`, `textures/azalea_icons/Comment`, (player)=>{
        uiManager.open(player, "add_comment", group)
    })
    actionForm.button(`§dAdd command call\n§7Run commands`, `textures/azalea_icons/Command`, (player)=>{
        uiManager.open(player, "add_command", group)
    })
    actionForm.button(`§dAdd if condition\n§7Conditionally conditioner`, `textures/azalea_icons/If`, (player)=>{
        uiManager.open(player, "add_if", group)
    })
    actionForm.button(`§dAdd function\n§7Reusable code`, `textures/azalea_icons/Function`, (player)=>{
        uiManager.open(player, "add_function", group)
    })
    actionForm.button(`§dCall function\n§7Call functions`, `textures/azalea_icons/ActionPink`, (player)=>{
...
uiManager.addUI("manage_instructions", "a", (player, group)=>{
    ...
    for(const instruction of instructions) {
        if(instruction.type == "comment") {
            actionGroup.button(instruction.label, `textures/azalea_icons/Comment`)
        }
    }
...
uiManager.addUI("actions_root", "a", (player)=>{
    ...
    for(const actionGroup of plijkokfn.db.data) {
        actionForm.button(actionGroup.data.name, `textures/azalea_icons/ActionRed`, (player)=>{
            uiManager.open(player, "manage_instructions", actionGroup.data.name)
        })
    }

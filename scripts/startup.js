...
if (system.beforeEvents.startup) {
    ...
    system.beforeEvents.startup.subscribe(async (init) => {
        init.blockComponentRegistry.registerCustomComponent("leaf:code_block", {
            ...
            onPlayerInteract(arg0, arg1) {
                ...
                if (arg0.player.isSneaking) {
                    let db = libPDB.positionalDb.getPosition(
                        arg0.block.location
                    );
        ...
        if (init.customCommandRegistry) {
            init.customCommandRegistry.registerEnum("leaf:femboy_type", [
                "uwu",
                "owo",
            ]);
            ...
            init.customCommandRegistry.registerCommand(
                {
                    ...
                    description:
                        "Save players inventory to the global inventory stash",
                    permissionLevel: CommandPermissionLevel.GameDirectors,
                    mandatoryParameters: [
                        {
                            name: "players",
            ...
            init.customCommandRegistry.registerCommand(
                {
                    ...
                    description:
                        "Load players inventory from global inventory stash",
                    permissionLevel: CommandPermissionLevel.GameDirectors,
                    mandatoryParameters: [
                        {
                            name: "players",
            ...
            init.customCommandRegistry.registerCommand(
                ...
                (origin, players, scriptevent) => {
                    system.run(() => {
                        for (const player of players) {
                            player.runCommand(
                                `scriptevent leaf:open ${scriptevent}`
                            );
                        }
                    });
                }
...
config.default.db.load().then(() => {
    if (
        ...
        !config.default.getProperty("Activated")
    ) {
        system.runTimeout(() => {

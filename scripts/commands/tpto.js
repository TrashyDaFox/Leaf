...
commandManager.addCommand("to", { description: "Teleport to someone without them knowing", author: "FruitKitty + TrashyKitty", category: "Admin", format: "!to <player>" }, ({ msg, args }) => {
    if (!msg.sender.hasTag('admin')) return msg.sender.error("You are not an admin!")
    let targetPlayer;
    for (const player of world.getPlayers()) {
        if (player.name.toLowerCase() == args[0].toLowerCase()) targetPlayer = player;
    }
    if (!targetPlayer) return msg.sender.error("That is not an online player!")
    if (targetPlayer.name == (`${msg.sender.name}`)) return msg.sender.error("You can't teleport to yourself, silly!") // i used lua syntax and somehow it worked

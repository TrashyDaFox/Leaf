...
class PlayerUtils {
    ...
    getStrawberryRanks(player) {
        let strawberryRanks = configAPI.getProperty("SingleRankMode") ? ranks.getRanks() : ranks.getRanks().reverse();
        let ranks2 = [];
        for(const rank of strawberryRanks) {
            if(!player.hasTag(rank.tag)) continue;
            ranks2.push(rank.name)
        }
        let ranks3 = ranks2.length ? ranks2 : [configAPI.getProperty("DefaultRank")];
        return configAPI.getProperty("SingleRankMode") ? ranks3.slice(0, 1) : ranks3;
    }
    ...
    getRanks(player) {
        if(configAPI.getProperty("UseNewRanks")) return this.getStrawberryRanks(player);
    ...
    getNameColor(player) {
        if(configAPI.getProperty("UseNewRanks")) return this.getStrawberryNameColor(player);
    ...
    getBracketColor(player) {
        if(configAPI.getProperty("UseNewRanks")) return this.getStrawberryBracketColor(player);
    ...
    getMessageColor(player) {
        if(configAPI.getProperty("UseNewRanks")) return this.getStrawberryMessageColor(player);

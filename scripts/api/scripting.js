...
class Scripting {
    ...
    #initializeHooks() {
        ...
        mc.world.beforeEvents.itemUse.subscribe(e=>{
            ...
            this.callHooks(e.source, `item-use:${e.itemStack.typeId.replace('minecraft:', '')}`, e)
        })
    }
    callHooks(entityScope = null, name, ...args) {
        this.hooks.forEach((hook)=>{
            ...
            if(entityScope) {
                let zone = zones.getZoneAtVec3ExcludeLandClaims(entityScope.location)
    ...
    hook(id) {
        return (type, callback, zoneLockdown = "")=>{
            let uniqueID = `hook_${(Date.now() * 1000) + Math.floor(Math.random() * 1000)}`
    ...
    setGlobal(id) {
        return (key, value, protected2)=> {
            ...
            this.globals[id][key] = {protected: protected2 ? protected2 : false, value}
        }
    }
    deleteGlobal(id) {
        return (key)=>{
    ...
    getGlobal(id) {
        return (key, scriptID = id)=> {
            ...
            if(this.globals[scriptID][key]) {
                if(this.globals[scriptID][key].protected && scriptID != id) return null;
    ...
    registerScript(id, script) {
        if(this.hookedScripts[id]) return mc.world.sendMessage(`Script tried registering twice, cancelling`)
        try {
            let fn = new Function(`return function ({callScriptHook, SegmentedStoragePrismarine, setGlobal, getGlobal, deleteGlobal, playerStorage, uiManager, actionParser, prismarineDb, icons, uiBuilder, homes, OpenClanAPI, formatStr, unhook, hook, mc, ui, reload}){\n${script}\n}`)()
            fn({
                mc,
                ui,
                playerStorage,
            ...
            mc.world.sendMessage(`§cLeaf Scripting API > Failed to hook function §e${id}§r§c.`)

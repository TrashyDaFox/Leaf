...
class LandClaims {
    constructor() {
        this.db = prismarineDb.customStorage("LandClaims", SegmentedStoragePrismarine)
        this.initSystem();
    }
    initSystem() {
        system.runInterval(()=>{
            ...
            for(const player of world.getPlayers()) {
                ...
                if((zone ? zone.id : null) != currZone) {
                    playerMap.set(player.id, zone ? zone.id : null)
                    player.onScreenDisplay.setActionBar(`${zone ? zone.data.name : "Wilderness"}`)
                }
            }
        }, 20)
    }
    isInClaim(pos) {
        ...
        for(const doc of this.db.findDocuments({type:"CLAIM"})) {
            let result = isPointInCube(pos.x, pos.y, pos.z, doc.data.pos1.x, doc.data.pos1.y, doc.data.pos1.z, doc.data.pos2.x, doc.data.pos2.y, doc.data.pos2.z)
    ...
    async claim(name, owner, pos1, pos2) {
        ...
        let increasingPrice = configAPI.getProperty("LandClaimsIncreasingPrice");
        let increasingPriceGrowthFactor = configAPI.getProperty("LandClaimsIncreasingPriceGrowthFactor");
        let landClaimCount = this.db.findDocuments({owner: playerID, ownerType: "PLAYER", type: "CLAIM"}).length;
        let regionSize = configAPI.getProperty("LandClaimsRegionSize")
        let price = Math.floor((
            basePrice + (Math.max(0, (Math.floor(size / (regionSize ** 2)) - 1)) * priceForRegion)
        ) + (increasingPrice == 0 ? 0 : Math.floor(landClaimCount > 0 ? ((landClaimCount ** increasingPriceGrowthFactor) * increasingPrice) : 0)))

        console.warn(JSON.stringify({
            basePrice,
            priceForRegion,
            regions: (Math.floor(size / (regionSize ** 2)) * priceForRegion),
            increasingPrice,
            increasingPriceGrowthFactor,
            size,
            maxArea,
            landClaimCount,
            price
        ...
        let playerBalance = prismarineDb.economy.getMoney(owner, currencyData.scoreboard)
        ...
        for(const player of world.getPlayers()) {
            try {
                ...
                if(id == claim.data.owner) {
                    prismarineDb.economy.addMoney(player, refundAmount, claim.data.currency)
                    foundPlayer = true;
                }
            } catch {}
        }
        if(!foundPlayer) {
            playerStorage.addReward(claim.data.owner, claim.data.currency, refundAmount)
        }

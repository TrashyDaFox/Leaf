...
uiManager.addUI(config.uiNames.IconViewer, "Icon Viewer", async (player, page = 0, callbackFn, favoritedOnly = false, iconIDSearch = false, iconIDSearchError = false, defaultIconID = null)=>{
    try {
        ...
        let rowCount = 5;
        for(let i = 0;i < 9;i++) {
            chest.button((9 * rowCount) + i, "§cX", [], "textures/blocks/glass_gray", 1, false, ()=>{
                uiManager.open(player, config.uiNames.IconViewer, page, callbackFn, favoritedOnly, iconIDSearch, iconIDSearchError, defaultIconID)
            })
        }
        // chest.button(9 * 5, "§cBack", ["Go back 1 page"], "textures/ui/arrow_left.png", 1, false, ()=>{
        chest.button(9 * rowCount, "§cBack", ["Go back 1 page"], "textures/blocks/glass_red", 1, false, ()=>{
        ...
        chest.button(callbackFn ? (9 * rowCount) + 3 : (9 * rowCount) + 3, "§dGo to page", ["Select a page to go to"], "textures/items/compass_item", 1, false, ()=>{
        ...
        // });
        if(callbackFn) {
            chest.button((9 * rowCount) + 1, "§cCancel", ["Cancel"], "textures/ui/cancel", 1, false, ()=>{
                callbackFn(player, null);
            });
    
            chest.button((9 * rowCount) + 2, "§dUse Icon ID", ["Manually input an icon ID"], "textures/items/spyglass", 1, false, ()=>{
                uiManager.open(player, config.uiNames.IconViewer, 0, callbackFn, favoritedOnly, true);
            });
            chest.button((9 * rowCount) + 6, "§eRecently Used", ["View recently used icons"], "textures/items/clock_item", 1, false, ()=>{
            ...
            chest.button((9 * rowCount) + 7, "§cRemove Icon", ["Remove the icon"], "textures/ui/icon_trash", 1, false, ()=>{
                callbackFn(player, "");
            });
            // textures/ui/icon_trash.png
        }
        chest.button((9 * rowCount) + 4, "§6Favorites", ["Favorited icons"], "textures/ui/sidebar_icons/star", tags.length, false, ()=>{
            uiManager.open(player, config.uiNames.IconViewer, 0, callbackFn, !favoritedOnly);
        });
        chest.button(callbackFn ? (9 * rowCount) + 5 : (9 * rowCount) + 5, favoritedOnly == "RECENTLY_USED" ? "§dClear Recently Used" : "§cClear Favorites", favoritedOnly == "RECENTLY_USED" ? ["Clear icons you have recently used"] : ["Clear all the icons you have favorited"], "textures/blocks/barrier", 1, false, ()=>{
        ...
        chest.button((9 * rowCount) + 8, "§aNext", ["Go next 1 page"], "textures/blocks/glass_lime", 1, false, ()=>{
            try {

...
class Ranks {
    constructor() {
        this.ranksDb = prismarineDb.customStorage("Ranks", SegmentedStoragePrismarine)
    ...
    createAdminRank() {
        if(!configAPI.getProperty("CreatedAdminRank")) {
            this.createRank("admin", "§nAdmin", "default", "default", "default", 1)
    ...
    createRank(tag, name, nameColor, bracketColor, messageColor, priority = 1) {
        ...
        let doc = {type: "RANK", tag, name, nameColor, bracketColor, messageColor, priority}
    ...
    getRanks() {
        let ranks = this.ranksDb.findDocuments({type:"RANK"}).map(_=>_.data).sort((a,b)=>{

...
export default [
    {
        ...
        callback(player, ui) {
            let currentTheme = themes.slice(1).findIndex(_=>ui.titleText.includes(_[0]));
            ui.title(`${ui.titleText.replaceAll(`${NUT_UI_THEMED}`, ``)}${currentTheme > -1 ? `${NUT_UI_THEMED}${themes[currentTheme][0]}` : ``}`)
    ...
    },
    {
        ...
        callback(player, ui) {
            ...
            modalForm.textField("Timestamp (Unix Timestamp)", "yes", form.updatedAt.toString(), (player)=>{
            })
            modalForm.show(player, false, (player, response)=>{
                let index = uiBuilder.db.data.findIndex(_=>_.id == form.id)
                if(index > -1) {
                    uiBuilder.db.data[index].updatedAt = parseInt(response.formValues[0]);
                }
                uiBuilder.db.save();

...
export class CommandHandler {
  ...
  static init() {
    world.beforeEvents.chatSend.subscribe((eventData) => {
      ...
      if (command && commandData) {
        ...
        // OPTION: Required tag
        if (commandData.config?.requiredPermission && !prismarineDb.permissions.hasPermission(eventData.sender, commandData.config.requiredPermission)) {
        ...
        if (commandData.subcommands && args.length > 0) {
          ...
          while (currentCommand.subcommands?.has(args[depth]?.toLowerCase())) {
            currentCommand = currentCommand.subcommands.get(args[depth].toLowerCase());
            depth++;
            
            // Check permissions for subcommand
            if (currentCommand.config?.requiredPermission && 
                !prismarineDb.permissions.hasPermission(sender, currentCommand.config.requiredPermission)) {
          ...
          if (depth > 0) {
            ...
            if (currentCommand.config?.validateArgs && !currentCommand.config.validateArgs(args)) {
              sender.sendMessage(
                `§cInvalid arguments! Use: ${currentCommand.config.structure?.replace("{cmd}", `!${command}`)}`
              );
              return;
            }
        ...
        if (commandData.config?.validateArgs && !commandData.config.validateArgs(args)) {
          sender.sendMessage(
            `§cInvalid arguments! Use: ${commandData.config.structure?.replace("{cmd}", `!${commandData.name}`)}`
          );
          return;
        }
        ...
        eventData.sender.sendMessage(`§cUnknown command! Use !help to see available commands.`);
      }
    });
  }
   ...
   */
  static registerSubCommand(commandName, subCommandName, callback, config = {}, nestedSubcommands = null) {
    const command = this.commands.get(commandName.toLowerCase());
    if (!command) {
      console.warn(`Cannot register subcommand for non-existent command: ${commandName}`);
      return;
    }
    ...
    const subCommandData = {
      ...
      subcommands: nestedSubcommands instanceof Map ? nestedSubcommands : null
    };
    ...
    this.registerCommand(
      ...
      (args, sender) => {
        ...
        if (args.length > 0) {
          ...
          for (let i = 1; i < commandPath.length && commandEntry?.subcommands; i++) {
            const subCmd = commandPath[i].toLowerCase();
            if (!commandEntry.subcommands.has(subCmd)) {
              sender.sendMessage("§cThe specified command/subcommand was not found!");
              return;
            }
          ...
          if (!commandEntry) {
            sender.sendMessage("§cThe specified command was not found!");
            return;
          }

          if (commandEntry.config?.requiredTag && !sender.hasTag(commandEntry.config.requiredTag)) {
            sender.sendMessage("§cYou don't have permission to view this command!");
            return;
          }

          const aliases = commandEntry.config?.aliases ? `§3 | ${commandEntry.config.aliases.join(" | ")}§b` : "";
          const messageParts = [
            `\n §b---- Command !${currentPath}${aliases} ----\n`,
            commandEntry.config?.description ? ` §f${commandEntry.config.description}\n` : "",
            commandEntry.config?.helpMessage ? ` §7${commandEntry.config.helpMessage}\n` : "",
            commandEntry.config?.structure
              ? ` §eStructure: §f${commandEntry.config.structure.replaceAll("{cmd}", `!${currentPath}`)}\n`
              : "",
          ...
          if (commandEntry.subcommands?.size > 0) {
            messageParts.push("\n §dSubcommands:\n");
            for (const [subName, subData] of commandEntry.subcommands) {
              messageParts.push(
                ` §5${subName}${
                  subData.config?.description ? ` §7- ${subData.config.description}` : ""
                }${
                  subData.subcommands?.size ? " §8(has subcommands)" : ""
                }\n`
              );
            }
          ...
          const sortedCommands = Array.from(this.commands.entries()).sort((a, b) => {
          ...
          sortedCommands.forEach(([cmd, data]) => {
            if (!data.config?.requiredTag || sender.hasTag(data.config?.requiredTag)) {
              helpMessageParts.push(
                `${data.config?.requiredTag ? ` §c(${data.config.requiredTag}) ` : " "}§a!${cmd}${
                  data.config?.description ? `§7: ${data.config.description}` : ""
                }${
                  data.subcommands?.size ? " §8(has subcommands)" : ""
                }\n`
              );
            }
          });
      ...
      },
      {
        ...
        helpMessage: "If a command is specified, shows help for that command and its subcommands.",
      }
    );
